...
#[test]
fn format_tests() {
	let bin = BinaryTranscoder::bincode();

	assert!(!bin.is_cbor());
	assert!(bin.is_bincode());
}
...
mod tests {
	...
	use crate::{
		fs::{transcoders::BinaryTranscoder, FsBackend, FsError},
	...
	async fn init() -> Result<(), FsError> {
		...
		let backend = FsBackend::new(BinaryTranscoder::bincode(), &path)?;
	...
	async fn table_methods() -> Result<(), FsError> {
		...
		let backend = FsBackend::new(BinaryTranscoder::cbor(), &path)?;
